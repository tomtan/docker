...
——容器的实质是进程，与宿主执行的进程不同，容器进程运行于自己独立的命名空间。容器可拥有自己的root文件系统、网络配置、进程空间、用户ID等。<br/>
——容器进程运行在一个隔离的环境，独立于宿主的系统下。<br/>
——镜像存储层是以镜像构建时的存储层作为读写，容器消亡时，容器存储层也随之消亡。<br/>
——最佳实践：容器不应向其存储层内写入任何数据，容器存储层要保持无状态化。<br/>
——文件写入操作：应使用数据卷Volume或绑定宿主目录/网络存储（性能和稳定性更高）。<br/>
——数据卷的生存周期独立于容器：容器消亡，数据卷不会消亡，数据不会丢失。<br/>
